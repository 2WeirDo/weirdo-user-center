...
const UserInfo: React.FC = () => {
  ...
  return (
    <>
      ...
      <Descriptions style={{ margin: '20px', marginLeft: '10px' }}>
        <Descriptions.Item>
          <Image
      ...
      <Descriptions bordered column={4}>
        ...
        <Descriptions.Item label="用户账户" span={4}>
          {myUser.userAccount}
        </Descriptions.Item>
        <Descriptions.Item label="用户角色" span={4}>
          {myUser.userRole === 0 ? '普通用户' : '管理员'}
        </Descriptions.Item>
        <Descriptions.Item label="用户性别" span={4}>
          {myUser.gender !== null ? (myUser.gender === 0 ? '男' : '女') : '未填写'}
        </Descriptions.Item>
        <Descriptions.Item label="创建时间" span={4}>
          {myUser.createTime}
        </Descriptions.Item>
        <Descriptions.Item label="用户状态" span={4}>
          {myUser.userStatus === 0 ? '正常' : '异常'}
        </Descriptions.Item>
        <Descriptions.Item label="用户电话" span={4}>
          {myUser.phone}
        </Descriptions.Item>
        <Descriptions.Item label="用户邮箱" span={4}>
          {myUser.email}
        </Descriptions.Item>
      </Descriptions>
      ...
      <ModalForm<API.CurrentUser>
        title="修改用户信息"
        trigger={
          <Button type="primary" shape="round" style={{ marginTop: '100px' }}>
            修改信息
          </Button>
        }
        autoFocusFirstInput
